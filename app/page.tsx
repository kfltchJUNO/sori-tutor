...
export default function Home() {
  ...
  const fetchInbox = async () => {
    ...
    if (unread.length > 0) {
      ...
      unread.forEach((m: any) => {
        const ref = doc(db, "sori_users", currentUser.email, "inbox", m.id);
  ...
  const saveNickname = async (newAlias: string) => {
    ...
    if (currentUser) {
       await updateDoc(doc(db, "sori_users", currentUser.email), { alias: trimmed });
       setUserAlias(trimmed);
       setShowNicknameModal(false);
       alert(`환영합니다, ${trimmed}님!`);
    }
  };
  ...
  const analyzeAudio = async () => {
    ...
    if (courseType === "dialogue" && targetLineIndex !== null) {
      ...
      contextInfo = `장소: ${selectedCategory}, 상황: ${currentProblem.title}, 역할: ${myRole}`;
    }
    ...
    try {
      ...
      if (data.error) {
        ...
        let newTodayCount = userData?.today_count || 0;
        ...
        if (lastDate !== todayStr) {
           ...
           if (lastDate === yesterday.toDateString() && userData?.today_count >= 5) {
             // 스트릭 유지
           } else {
             newStreak = 0; // 끊김
           }
        ...
        if (newTodayCount === 5) {
          ...
          if (newStreak > 0 && newStreak % 7 === 0) {
             rewardMsg = `🔥 ${newStreak}일 연속 달성! 보상으로 토큰 15개를 드립니다! 🎁`;
             if (userRole === 'student') {
               await updateDoc(userRef, { tokens: increment(15) });
               setTokens(prev => prev + 15);
             }
          } else {
             rewardMsg = `🔥 오늘의 목표 달성! (${newStreak}일 연속 성공)`;
          }
        }
        ...
        await addDoc(collection(db, "sori_users", currentUser.email, "history"), {
          text: targetText, category: courseType==="word"?currentProblem.category:selectedCategory, score: data.score, feedback: data.feedback, type: courseType, date: serverTimestamp()
        });
      }
  ...
  const getGrammarExplanation = async (hId: string, text: string, type: string) => {
    ...
    if (!confirm(userRole === 'guest' ? "하트 1개가 차감됩니다." : "토큰 1개가 차감됩니다.")) return;
...
(이곳에 내용을 적어주세요)`;
  ...
  const selectCourse = async (type: any) => { 
    ...
    if (type === "word") {
      ...
      setProblemList(l);
      if (l.length > 0) pickRandomProblem(l);
  ...
  const selectCategory = async (cat: string) => { 
    ...
    setProblemList(l); 
    if (l.length > 0) { 
      setCurrentProblem(l[0]); 
      if(courseType === "dialogue") parseDialogue((l[0] as any).script); 
    } 
  ...
  const pickRandomProblem = (l: any[] = problemList) => { if (l.length === 0) return; const r = Math.floor(Math.random() * l.length); setCurrentProblem(l[r]); setResult(null); setAudioUrl(null); setAudioBlob(null); };
  ...
  if (!currentUser) return (
    <main className="flex min-h-screen items-center justify-center bg-gray-50 p-6">
      <div className="text-center bg-white p-10 rounded-2xl shadow-xl max-w-sm w-full">
        <h1 className="text-4xl font-black text-gray-900 mb-2">Sori-Tutor</h1>
        <p className="text-gray-700 mb-6 font-medium">AI 음운론 기반 발음 교정</p>
        <Login onUserChange={handleUserChange} />
        <p className="text-sm text-gray-600 mt-4">* 로그인 시 무료 체험(일 3회)</p>
      </div>
    </main>
  );
    ...
    <main className="flex min-h-screen flex-col bg-gray-50 max-w-lg mx-auto shadow-2xl relative">
      ...
      <header className="bg-white px-5 py-4 flex justify-between items-center sticky top-0 z-40 border-b border-gray-200">
        ...
        <div className="flex items-center gap-3">
           {/* 📩 쪽지함 버튼 */}
           <button onClick={fetchInbox} className="relative text-2xl mr-2">
             📩
           ...
           {/* 💰 재화 버튼 */}
           <div className="flex items-center gap-1 bg-gray-100 px-3 py-1 rounded-full text-sm font-bold cursor-pointer border border-gray-300 hover:bg-gray-200" onClick={() => setShowPaymentModal(true)}>
             {userRole === 'guest' ? (<><span className="text-red-500">❤️</span><span className="text-gray-900">{hearts}</span></>) : (<><span className="">🪙</span><span className="text-gray-900">{tokens}</span></>)}
             <span className="text-xs text-gray-600 ml-1 font-normal">충전</span>
           </div>
      ...
      <div className="p-6 flex-1 overflow-y-auto pb-32">
          ...
          <div className="space-y-4 animate-fade-in-up">
            <div className="bg-blue-50 p-5 rounded-xl border border-blue-200 mb-4 shadow-sm">
               ...
               <div className="bg-white p-3 rounded-lg border border-blue-100 flex items-center justify-between shadow-sm">
                  <div>
                    <p className="text-xs text-gray-500 font-bold mb-1">오늘의 목표 (5회)</p>
                    <div className="flex gap-1">
                      {[1,2,3,4,5].map(i => (
                        <div key={i} className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold ${todayCount >= i ? 'bg-green-500 text-white' : 'bg-gray-200 text-gray-400'}`}>
                          {todayCount >= i ? '✓' : i}
                        </div>
                      ))}
                    </div>
                  ...
                  <div className="text-right">
                     <p className="text-2xl font-black text-orange-500">🔥 {streak}일</p>
                     <p className="text-xs text-orange-800 font-bold">연속 학습 중!</p>
                  </div>
               </div>
            </div>
            ...
            {[{id:'word', t:'단어 카드', d:'랜덤 발음 연습', c:'blue'}, {id:'sentence', t:'문장 억양', d:'필수 문장 연습', c:'indigo'}, {id:'dialogue', t:'실전 회화', d:'역할극 대화 연습', c:'purple'}].map(item => (
              <button key={item.id} onClick={() => selectCourse(item.id as any)} 
                className={`w-full p-6 rounded-2xl text-left bg-white shadow-md border-2 border-transparent hover:border-${item.c}-500 transition group`}>
        ...
        {viewMode === "category" && (
          <div>
            ...
            <div className="grid grid-cols-2 gap-4">
              {categories.map(cat => (
                <button key={cat} onClick={() => selectCategory(cat)} className="bg-white p-5 rounded-xl shadow border-2 border-gray-100 hover:border-blue-500 font-bold text-gray-800 text-lg">{cat}</button>
              ))}
            </div>
          </div>
        )}
            ...
            <div className="flex gap-2 mb-6 overflow-x-auto pb-2">
              {['all', 'word', 'sentence', 'dialogue'].map(tab => (
                 <button key={tab} onClick={() => setHistoryTab(tab as any)}
                   className={`px-4 py-2 rounded-full font-bold text-sm whitespace-nowrap transition ${historyTab === tab ? 'bg-gray-900 text-white' : 'bg-white text-gray-600 border border-gray-300'}`}>
              ...
              {historyList.filter(h => historyTab === 'all' || h.type === historyTab).map((h, i) => (
                <div key={i} className="bg-white p-5 rounded-xl shadow-sm border border-gray-200">
                  <div className="flex justify-between items-center mb-3">
                    <span className="text-xs font-bold bg-gray-100 px-2 py-1 rounded text-gray-700 border border-gray-200">{h.type} / {h.category}</span>
                    <span className="text-xs text-gray-500">{h.date?.toDate ? h.date.toDate().toLocaleDateString() : ""}</span>
                  </div>
                  <div className="font-black text-gray-900 text-xl mb-3">"{h.text}"</div>
                  <div className="flex items-center gap-2 mb-3">
                    <span className={`text-2xl font-black ${Number(h.score) >= 80 ? 'text-green-600' : 'text-orange-500'}`}>{h.score}점</span>
                    <p className="text-sm text-gray-800 bg-gray-50 p-2 rounded flex-1 border border-gray-100 font-medium">{h.feedback}</p>
                  </div>
                  <button onClick={() => getGrammarExplanation(h.id, h.text, h.type)} className="text-xs font-bold text-blue-600 hover:text-blue-800 flex items-center gap-1">
                      {expandedExplanationId === h.id ? "접기 ▲" : `🔍 문법 설명 보기 (${userRole === 'guest' ? '❤️-1' : '🪙-1'})`}
                  </button>
        ...
        {viewMode === "practice" && currentProblem && (
          <div className="flex flex-col h-full">
            {courseType !== "word" && <button onClick={() => setViewMode("category")} className="mb-2 text-gray-500 font-bold text-sm">← {selectedCategory}</button>}
            {courseType === "word" && <button onClick={() => setViewMode("home")} className="mb-2 text-gray-500 font-bold text-sm">← 종료</button>}
            ...
            {courseType === "dialogue" ? (
              <div className="space-y-4">
                <div className="bg-purple-50 p-5 rounded-xl border border-purple-100 shadow-sm">
                  ...
                  <div className="flex bg-white rounded-lg p-1 border border-purple-200 w-fit mb-2">
                    <button onClick={() => setMyRole("A")} className={`px-4 py-1 rounded text-sm font-bold transition ${myRole==="A"?'bg-purple-700 text-white':'text-gray-500'}`}>A</button>
                    <button onClick={() => setMyRole("B")} className={`px-4 py-1 rounded text-sm font-bold transition ${myRole==="B"?'bg-purple-700 text-white':'text-gray-500'}`}>B</button>
                  </div>
                      ...
                      <div key={idx} className={`flex ${isMyRole ? 'justify-end' : 'justify-start'}`}>
                        <div onClick={() => { if (isMyRole) { setTargetLineIndex(idx); setResult(null); setAudioUrl(null); setAudioBlob(null); }}}
                          className={`max-w-[85%] p-4 rounded-2xl cursor-pointer border-2 transition-all 
                            ${isMyRole ? (isSelected ? 'bg-blue-100 border-blue-600 shadow-md scale-105' : 'bg-white border-blue-100 hover:border-blue-300') : 'bg-gray-100 border-transparent text-gray-500'}`}>
                          <div className="text-xs font-bold mb-1 opacity-70 text-gray-800">{line.role}</div>
                          <div className={`text-lg leading-snug font-bold ${isMyRole ? 'text-gray-900' : 'text-gray-600'}`}>{line.text}</div>
                          {isSelected && !result && <div className="text-xs text-blue-700 font-bold mt-2 animate-pulse">👇 녹음 버튼을 누르세요</div>}
                        </div>
                      </div>
                    );
                  })}
              ...
              <div className="bg-white rounded-3xl shadow-lg border border-gray-200 p-8 text-center mb-6 relative">
                 ...
                 <div className="bg-gray-100 text-gray-800 text-base font-medium p-3 rounded-xl inline-block border border-gray-200">
          ...
          {result ? (
            <div className="animate-fade-in-up">
              <div className="flex items-center gap-4 mb-4">
                <div className={`text-5xl font-black ${result.score >= 80 ? 'text-green-600' : 'text-orange-500'}`}>{result.score}</div>
                <div className="flex-1 text-base text-gray-800 bg-gray-50 p-4 rounded-xl border border-gray-200 font-medium">{result.feedback}</div>
              </div>
              <button onClick={() => {setResult(null); setAudioUrl(null); if(courseType==="word") pickRandomProblem();}} 
                className="w-full py-4 bg-gray-900 text-white rounded-xl font-bold shadow-lg hover:bg-gray-800 transition text-lg">
                {courseType === "word" ? "다음 단어 🎲" : "확인 (계속하기)"}
              </button>
            </div>
          ) : (
            <div className="flex flex-col items-center gap-4">
               {courseType === "dialogue" && targetLineIndex === null && <div className="text-sm text-blue-700 font-bold bg-blue-100 px-4 py-2 rounded-full animate-bounce shadow-sm">☝️ 내 대사를 먼저 터치하세요!</div>}
      ...
      {/* 닉네임 설정 모달 (수정됨: 취소 버튼 추가) */}
      {showNicknameModal && (
        <div className="fixed inset-0 bg-black/70 z-[100] flex items-center justify-center p-4">
          <div className="bg-white p-6 rounded-2xl w-full max-w-sm text-center">
            <h2 className="text-2xl font-black mb-2 text-gray-900">닉네임 설정</h2>
            <p className="text-sm text-gray-600 mb-4">랭킹에 표시될 멋진 이름을 지어주세요!</p>
            <input className="w-full border-2 p-3 rounded-xl mb-4 font-bold text-center text-gray-900" value={userAlias} onChange={e => setUserAlias(e.target.value)} />
            <div className="flex gap-2">
               ...
               {/* ❌ 취소 버튼 추가 */}
               <button onClick={() => setShowNicknameModal(false)} className="flex-1 py-3 bg-gray-200 text-gray-700 font-bold rounded-xl hover:bg-gray-300">취소</button>
            </div>
          </div>
        </div>
      )}
      ...
      {showInboxModal && (
        <div className="fixed inset-0 bg-black/70 z-[100] flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl w-full max-w-sm max-h-[500px] overflow-hidden flex flex-col">
            <div className="bg-blue-600 p-4 text-white font-bold text-center relative">
              📬 도착한 메시지함
              <button onClick={() => setShowInboxModal(false)} className="absolute right-4 top-4 text-white">✕</button>
            </div>
            <div className="p-4 overflow-y-auto flex-1 bg-gray-50 space-y-3">
              {inboxList.length === 0 ? (
                <p className="text-gray-500 text-center py-10">도착한 메시지가 없어요.</p>
              ) : (
                inboxList.map((msg) => (
                  <div key={msg.id} className="bg-white p-4 rounded-xl shadow-sm border border-gray-200">
                    <div className="flex justify-between items-center mb-2">
                      <span className="text-xs font-bold text-blue-600 bg-blue-50 px-2 py-1 rounded">{msg.from || "관리자"}</span>
                      <span className="text-xs text-gray-400">{msg.date?.toDate ? msg.date.toDate().toLocaleDateString() : ""}</span>
                    </div>
            ...
            <div className="p-4 border-t bg-white">
              <button onClick={() => setShowInboxModal(false)} className="w-full py-3 bg-gray-200 text-gray-700 font-bold rounded-xl">닫기</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 bg-black/70 z-[100] flex items-center justify-center p-4">
           <div className="bg-white rounded-2xl shadow-2xl max-w-sm w-full overflow-hidden">
             <div className="bg-yellow-400 p-4 text-center">
               <h2 className="text-2xl font-black text-yellow-900">🔥 명예의 전당</h2>
               <p className="text-xs text-yellow-800 font-bold">열공 중인 상위 10명</p>
             </div>
             <div className="p-4 max-h-[400px] overflow-y-auto">
               {rankingList.map((user, idx) => (
                 <div key={idx} className={`flex justify-between items-center p-3 mb-2 rounded-xl border-2 ${idx<3 ? 'border-yellow-200 bg-yellow-50' : 'border-gray-100'}`}>
                    <div className="flex items-center gap-3">
                      <span className={`font-black text-lg w-6 text-center ${idx===0?'text-yellow-600':idx===1?'text-gray-500':idx===2?'text-orange-600':'text-gray-400'}`}>{idx+1}</span>
                      <span className="font-bold text-gray-800">{user.alias || user.name}</span>
                    </div>
                    <span className="text-sm font-bold text-blue-600">{user.analysis_count || 0}회</span>
                 </div>
               ))}
             </div>
             <div className="p-4 border-t">
               <button onClick={() => setShowRankingModal(false)} className="w-full py-3 bg-gray-900 text-white font-bold rounded-xl">닫기</button>
             </div>
           </div>
        </div>
      )}
          ...
          <div className="bg-white rounded-2xl w-full max-w-md overflow-hidden">
             <div className="bg-gray-900 p-6 text-white text-center">
               <h2 className="text-2xl font-bold">
                 {(userRole === 'guest' && hearts <= 0) || (userRole === 'student' && tokens <= 0) ? "충전이 필요해요 😢" : "멤버십 / 토큰 충전 🔋"}
               </h2>
             </div>
             <div className="p-6">
               <div className="text-center mb-6">
                 {(userRole === 'guest' && hearts <= 0) || (userRole === 'student' && tokens <= 0) ? (
                   <p className="text-gray-700 font-medium">이용 횟수를 모두 사용하셨습니다.<br/>전문적인 코칭을 위해 충전해보세요!</p>
                 ) : (
                   <div className="bg-gray-50 p-4 rounded-xl border border-gray-200">
                     <p className="text-gray-500 text-sm mb-1">현재 보유 잔액</p>
                     <p className="text-2xl font-black text-blue-600 mb-2">{userRole === 'guest' ? `${hearts} 하트 ❤️` : `${tokens} 토큰 🪙`}</p>
                     <p className="text-gray-800 font-bold text-sm">추가 충전을 진행하시겠습니까?</p>
                   </div>
                 )}
               </div>
               <div className="space-y-3 mb-4">
                  <a href={getMailtoLink("Basic Plan", "5,500원")} className="block border-2 border-gray-100 hover:border-blue-500 rounded-xl p-4 transition group">
                    <div className="flex justify-between items-center">
                      <div className="text-left"><div className="font-bold text-lg text-gray-900 group-hover:text-blue-600">Basic Plan</div><div className="text-sm text-gray-500">🪙 토큰 100개 충전</div></div>
                      <div className="text-xl font-bold text-blue-600">5,500원</div>
                    </div>
                  </a>
                  <a href={getMailtoLink("Pro Plan", "9,900원")} className="block border-2 border-blue-100 bg-blue-50 hover:border-blue-600 rounded-xl p-4 transition group relative">
                     <div className="absolute top-0 right-0 bg-blue-600 text-white text-[10px] px-2 py-0.5 rounded-bl-lg font-bold">BEST</div>
                     <div className="flex justify-between items-center">
                      <div className="text-left"><div className="font-bold text-lg text-gray-900 group-hover:text-blue-700">Pro Plan</div><div className="text-sm text-gray-500">🪙 토큰 250개 (보너스!)</div></div>
                      <div className="text-xl font-bold text-blue-600">9,900원</div>
                    </div>
                  </a>
               </div>
               <button onClick={() => setShowPaymentModal(false)} className="w-full py-3 bg-gray-200 text-gray-700 font-bold rounded-xl hover:bg-gray-300 transition">닫기</button>
             </div>
          </div>
        </div>
      )}
